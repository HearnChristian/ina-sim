...
| Date | Note |
|------|------|
| 2026-07-25 | **v0.4.1** Hardening for external eyes: `ina-sim doctor` (environment, registry monotonicity, validation anchors, derived-fit and doc freshness, audit chain — one command, non-zero exit on failure); in-process CLI tests taking cli.py from an unmeasured 0% to 83% (it was always tested, just via subprocesses coverage cannot see); branch coverage measured and gated at 75%; CI now builds a wheel, installs it into a clean venv and runs the CLI from it; CITATION.cff, CHANGELOG.md, pre-commit hooks, PR template, README badges. 499 tests |
| 2026-07-25 | **v0.4.0** `ina-sim scenario` (payload → delivered INP → go/no-go, with the Monte Carlo folded in and a claims guardrail; solves backwards for payload) and `ina-sim history` (hash-chained audit log with a parameterization fingerprint, so `--diff` attributes a moved number to changed conditions vs changed science). GUI: reference figures moved to View, cursor tips to Help, T-sweep to Physics with a quantity selector (score / activation / INP per litre). **Fixed two defects I shipped in v0.3.4**: a measured material outside its fitted range silently fell back to the heuristic ns (7 orders of magnitude jump at the boundary — the "random-looking" sweep), and rate coefficients (kaolinite, pure water) were being used as site densities. Both now return no value, with regression tests |
